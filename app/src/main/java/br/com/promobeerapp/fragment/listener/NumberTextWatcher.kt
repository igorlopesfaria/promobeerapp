...
class NumberTextWatcher(private val et: EditText, pattern: String) : TextWatcher {
    ...
    override fun afterTextChanged(s: Editable?) {
        ...
        if (s != null && !s.toString().isEmpty()) {
            try {
                ...
                val v = s.toString().replace(df.getDecimalFormatSymbols().getGroupingSeparator().toString(), "").replace("$", "")
                val n = df.parse(v)
    ...
    override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
        var index = s.toString().indexOf(df.getDecimalFormatSymbols().getDecimalSeparator().toString())
        trailingZeroCount = 0
        if (index > -1) {
            index++
